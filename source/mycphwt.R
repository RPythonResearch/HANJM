...
mycphwt=function(formula,data,digits=2){
  ...
  for(i in 1:count) {
    ...
    suppressWarnings(out<-summary(survival::coxph(as.formula(s),data, weights=ate.weights)))
...
###ATT weight##
mycphattwt=function(formula,data,digits=2){
  call=paste(deparse(formula),", ","data= ",substitute(data),sep="")
  cat("\n mycph : perform coxph of individual expecting variables\n")
  cat("\n Call:",call,"\n\n")
  f=formula
  ...
  for(i in 1:count) {
    s=paste(y,myvar[i],sep="~")
    suppressWarnings(out<-summary(survival::coxph(as.formula(s),data, weights=att.weights)))
    if(any(is.infinite(out$conf.int))){
      cat(dimnames(out$conf.int)[[1]]," was excluded : infinite\n")
      next
    }
...
#'         the p values.
#'
extractHRwt=function(x,digits=3){
  digits=3
  out=summary(x)
  res=data.frame(round(out$conf.int[,-2],digits))
  res=cbind(res,round(out$coef[,6],max(3,digits)))
...
#'
HRplotwt=function(out,type=1,xlab="",ylab="",show.OR=TRUE,show.CI=FALSE,
                sig.level=1,cex=1.2,lwd=2,pch=18,col=NULL,...){
  if(class(out)=="coxph") res=extractHRwt(out)
  else res=out
